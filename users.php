...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'created')
        $user_message = "<div class='alert alert-success'><i data-feather='check-circle' style='width:16px;height:16px;'></i> User berhasil ditambahkan.</div>";
    if ($_GET['msg'] === 'edited')
        $user_message = "<div class='alert alert-success'><i data-feather='check-circle' style='width:16px;height:16px;'></i> Data user berhasil diperbarui.</div>";
    if ($_GET['msg'] === 'deleted')
        $user_message = "<div class='alert alert-success'><i data-feather='check-circle' style='width:16px;height:16px;'></i> User berhasil dihapus permanen.</div>";
}
...
<body>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if (mysqli_num_rows($usersUrl) > 0): ?>
                                <?php while ($u = mysqli_fetch_assoc($usersUrl)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge"
                                                style="background:<?php echo $u['role'] === 'admin' ? '#fef3c7' : 'var(--primary-light)'; ?>; color:<?php echo $u['role'] === 'admin' ? '#d97706' : 'var(--primary)'; ?>; padding:0.25rem 0.6rem; border-radius:99px; font-weight:600; font-size:0.8rem;">
                                        ...
                                        <td>
                                            <div style="display:flex; gap:0.5rem; justify-content:flex-end;">
                                                <button
                                                    onclick="openEditModal(<?php echo $u['id']; ?>, '<?php echo addslashes(htmlspecialchars($u['username'])); ?>', '<?php echo addslashes(htmlspecialchars($u['password'])); ?>', '<?php echo addslashes(htmlspecialchars($u['role'])); ?>')"
                                                    class="btn btn-warning btn-icon" title="Edit User">
                                                    <i data-feather="edit-2" style="width:14px;height:14px;"></i>
                                                </button>
                                                <button
                                                    onclick="openDeleteModal(<?php echo $u['id']; ?>, '<?php echo addslashes(htmlspecialchars($u['username'])); ?>')"
                                                    class="btn btn-danger btn-icon" title="Hapus User" <?php echo ($u['id'] == $_SESSION['user_id']) ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : ''; ?>>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="4"
                                        style="text-align:center; padding: 2rem; color:var(--text-light); font-style:italic;">
        ...
        <div class="modal-box">
            ...
            <form method="POST" action="users.php" id="buat-form">
                ...
                <div class="form-group" style="text-align:left; margin-top:1rem;">
                    <label for="username_buat"
                        style="font-weight:600; font-size:0.85rem; margin-bottom:0.4rem; display:block;">Username</label>
                    <input type="text" name="username" id="username_buat" class="form-control" required
                        placeholder="Masukkan username" autocomplete="off">
                </div>
                <div class="form-group" style="text-align:left; margin-top:1rem;">
                    <label for="password_buat"
                        style="font-weight:600; font-size:0.85rem; margin-bottom:0.4rem; display:block;">Password</label>
                    <input type="text" name="password" id="password_buat" class="form-control" required
                        placeholder="Masukkan password" autocomplete="off">
                </div>
                <div class="form-group" style="text-align:left; margin-top:1rem;">
                    <label for="role_buat"
                        style="font-weight:600; font-size:0.85rem; margin-bottom:0.4rem; display:block;">Role</label>
            ...
            <form method="POST" action="users.php" id="edit-form">
                ...
                <div class="form-group" style="text-align:left; margin-top:1rem;">
                    <label for="password_edit"
                        style="font-weight:600; font-size:0.85rem; margin-bottom:0.4rem; display:block;">Password</label>
                    <input type="text" name="password" id="password_edit" class="form-control" required
                        autocomplete="off">
                </div>
                <div class="form-group" style="text-align:left; margin-top:1rem;">
                    <label for="role_edit"
    ...
    <script>
        ...
        const modals = ['buat-modal', 'edit-modal', 'delete-modal'];
